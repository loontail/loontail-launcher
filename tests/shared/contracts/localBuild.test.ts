...
describe('LocalBuildManifestSchema', () => {
  it('fills defaults for omitted optional fields', () => {
    const parsed = LocalBuildManifestSchema.parse(minimalManifest);
  ...
  it('round-trips a fully-specified remote-bundle manifest', () => {
    ...
    const parsed = LocalBuildManifestSchema.parse(full);
  ...
  it('rejects a non-uuid id', () => {
    expect(
      LocalBuildManifestSchema.safeParse({ ...minimalManifest, id: 'not-a-uuid' }).success,
    ).toBe(false);
  });

  it('rejects an empty name', () => {
    expect(LocalBuildManifestSchema.safeParse({ ...minimalManifest, name: '' }).success).toBe(
      false,
    );
  });
  ...
  it('rejects an unknown loader type', () => {
    expect(
      LocalBuildManifestSchema.safeParse({
        ...minimalManifest,
...
describe('LocalBuildRegistrySchema', () => {
  it('defaults to an empty instances list', () => {
    expect(LocalBuildRegistrySchema.parse({ schema: 1 }).instances).toEqual([]);
  });

  it('parses a populated index', () => {
    const parsed = LocalBuildRegistrySchema.parse({
      schema: 1,
      instances: [
...
describe('CreateBuildPayloadSchema', () => {
  it('accepts a minimal create payload', () => {
    const parsed = CreateBuildPayloadSchema.parse({
      name: 'Fresh',
      minecraftVersion: '1.21.4',
      loader: 'vanilla',
    });
  ...
  it('rejects a blank name', () => {
    expect(
      CreateBuildPayloadSchema.safeParse({
        name: '',
        minecraftVersion: '1.21.4',
        loader: 'vanilla',
      }).success,
