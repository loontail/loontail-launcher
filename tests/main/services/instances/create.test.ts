...
import {
  type InstanceMutationDeps,
  createInstance,
  deleteInstance,
  listLoaderVersionOptions,
  updateInstance,
} from '@main/services/instances/create';
...
const captureDeps = () => {
  const saved: { dir: string; manifest: InstanceManifest }[] = [];
  const entries: InstanceRegistryEntry[] = [];
  const deps: InstanceMutationDeps = {
...
describe('createInstance', () => {
  it('pins the fabric loader version and writes manifest + registry under the install root', async () => {
    ...
    const item = await createInstance(
      fakeKit(),
...
describe('deleteInstance', () => {
  it('removes the registry entry and the instance directory', async () => {
    const removeEntry = vi.fn();
    const removeDir = vi.fn(async () => {});
    await deleteInstance(UUID as InstanceId, {
      getEntry: () => ({
...
describe('updateInstance', () => {
  const baseManifest: InstanceManifest = {
    schema: 1,
    id: UUID as InstanceId,
    name: 'Old',
  ...
  it('applies a name + description patch and bumps updatedAt', async () => {
    const writes: InstanceManifest[] = [];
    const item = await updateInstance(
      { id: UUID as InstanceId, patch: { name: 'New', description: 'fresh' } },
      {
        now: () => '2026-06-08T00:00:00.000Z',
        getEntry: () => ({ id: UUID as InstanceId, name: 'Old', dir: 'Z:/x', updatedAt: NOW }),
