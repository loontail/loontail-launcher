...
describe('renderClientDescriptionMarkdown', () => {
  it('renders ordinary markdown with safe external links', () => {
    const markdown = [
      ...
      '[News](https://api.loontail.tld/news "Read news")',
    ].join('\n\n');
    ...
    expect(rendered).toContain(
      '<a href="https://api.loontail.tld/news" title="Read news" target="_blank" rel="noreferrer noopener">News</a>',
    );
  });
  ...
  it('removes raw HTML, image tags, and unsafe URL protocols', () => {
    const markdown = [
      ...
      '<img src="https://api.loontail.tld/x.png" onerror="alert(1)">',
      '<a href="https://api.loontail.tld/news" onclick="alert(1)">raw link</a>',
      '[script link](javascript:alert(1))',
      '[file link](file:///C:/Users/secrets)',
      '![remote image](https://api.loontail.tld/uploads/image.webp)',
    ].join('\n\n');
  ...
  it('allows only explicitly approved https hosts', () => {
    const markdown = [
      '[allowed](https://cdn.api.loontail.tld/patch-notes)',
      '[offsite](https://example.com/patch-notes)',
      '[http](http://api.loontail.tld/patch-notes)',
    ].join('\n\n');

    const rendered = renderClientDescriptionMarkdown(markdown);

    expect(rendered).toContain('href="https://cdn.api.loontail.tld/patch-notes"');
    expect(rendered).not.toContain('href="https://example.com/patch-notes"');
    expect(rendered).not.toContain('href="http://api.loontail.tld/patch-notes"');
