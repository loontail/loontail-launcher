import fs from 'node:fs/promises';
import path from 'node:path';
import { errorMessage } from '@main/infra/errorMessage';
import type { CatalogKey } from '@shared/contracts/ids';
import { InstallStatuses, MinecraftErrorCodes } from '@shared/contracts/minecraft';
import type { MinecraftEnv } from './env';
import { MinecraftError } from './errors';
import { hasAnyVersionInstalled } from './installedVersions';
import { clearTargetInstallManifest } from './installManifest';
import { OpKinds } from './ops';

// Guard before recursive rm: reject anything not strictly under clientsRoot (no
// `..` escapes, no absolute paths).
export const isUnderClientsRoot = (folder: string, clientsRoot: string): boolean => {
  if (!folder || !clientsRoot) return false;
  const rel = path.relative(clientsRoot, folder);
  if (rel === '') return false;
  if (rel.startsWith('..')) return false;
  if (path.isAbsolute(rel)) return false;
  return true;
};

export const runUninstall = async (
  env: MinecraftEnv,
  key: CatalogKey,
  folder: string,
  clientsRoot: string,
): Promise<void> => {
  if (!isUnderClientsRoot(folder, clientsRoot)) {
    throw new MinecraftError(MinecraftErrorCodes.UNKNOWN, 'Refusing to delete suspicious path');
  }
  env.ops.set(key, { kind: OpKinds.UNINSTALL });
  env.emitStatus({ key, status: InstallStatuses.UNINSTALLING, paused: false });
  try {
    await fs.rm(folder, { recursive: true, force: true });
    await clearTargetInstallManifest(folder);
    env.clearRuntimeOverride(key);
    env.forgeProcessorCache.evictByDirectory(folder);
    env.emitStatus({ key, status: InstallStatuses.NOT_INSTALLED, paused: false });
  } catch (error) {
    env.logger.error(`[${key}] uninstall failed`, error);
    // Best-effort second pass on just the markers hasAnyVersionInstalled checks, so a
    // file lock elsewhere in the tree doesn't leave an ERROR + INSTALLED flicker.
    await Promise.allSettled([
      fs.rm(path.join(folder, 'versions'), { recursive: true, force: true }),
      fs.rm(path.join(folder, 'runtime'), { recursive: true, force: true }),
      clearTargetInstallManifest(folder),
      env.clearBundleManifest(folder),
    ]);
    if (!(await hasAnyVersionInstalled(folder))) {
      env.logger.warn(`[${key}] uninstall: residual files remain but markers are gone`);
      env.clearRuntimeOverride(key);
      env.forgeProcessorCache.evictByDirectory(folder);
      env.emitStatus({ key, status: InstallStatuses.NOT_INSTALLED, paused: false });
    } else {
      env.emitError(key, MinecraftErrorCodes.UNINSTALL_LOCKED, errorMessage(error));
      env.emitStatus({ key, status: InstallStatuses.ERROR, paused: false });
    }
  } finally {
    env.ops.delete(key);
  }
};
