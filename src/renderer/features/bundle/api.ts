...
export const start = (key: CatalogKey, force?: boolean): Promise<void> =>
  window.api.invoke(IPC_CHANNELS.bundleStart, { key, ...(force ? { force } : {}) });
...
export const getStatus = (key: CatalogKey): Promise<BundleInstallState> =>
  window.api.invoke(IPC_CHANNELS.bundleGetStatus, key);
