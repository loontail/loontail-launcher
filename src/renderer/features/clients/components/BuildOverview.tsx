...
const SectionLabel = ({ children, aside }: { children: string; aside?: ReactNode }) => (
  <div className="mb-3 flex items-center justify-between gap-3">
    <h3 className="text-microlabel font-bold uppercase tracking-eyebrow text-text-mute">
      {children}
    </h3>
    {aside}
  </div>
...
export const BuildOverview = ({ item, onViewScreenshots }: BuildOverviewProps) => {
  ...
  const loaderLabel = `${t(`clientSettings.loader.${loader}`)}${loaderVersion ? ` ${loaderVersion}` : ''}`;
  ...
  if (description) {
    ...
    aboutBody = <p className="text-body text-text-mute">{t('clients.noAbout')}</p>;
  }

  return (
    <div className="flex flex-col gap-8 pt-7">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile label={t('clients.stat.minecraft')} value={minecraftVersion} />
        <StatTile label={t('clients.stat.loader')} value={loaderLabel} />
        <StatTile label={t('clients.stat.servers')} value={servers.length} />
        <StatTile label={t('clients.stat.screenshots')} value={screenshots} />
      </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {previews.map((index) => (
                <button
                  ...
                  type="button"
                  onClick={onViewScreenshots}
                  aria-label={t('clients.screenshots')}
                  className={cn(
