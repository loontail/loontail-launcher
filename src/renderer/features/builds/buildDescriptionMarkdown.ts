...
// Inlined at build time by electron-vite's renderer `define` block. Vite only
// substitutes literal property accesses, so each read must spell out
// `process.env.<NAME>` — `process.env[name]` stays undefined in the bundle.
const configuredApiOrigin = process.env.API_URL;
const configuredExtraHosts = process.env.DESCRIPTION_LINK_HOSTS;

const hostnameOf = (rawUrl: string | undefined): string | null => {
...
// Empty entries are filtered out on purpose: an empty allowed host would make
// the `.${host}` suffix rule below match every dotted hostname on the internet.
const DESCRIPTION_LINK_ALLOWED_HOSTS: readonly string[] = Object.freeze(
  [...new Set([hostnameOf(configuredApiOrigin), ...parseHostList(configuredExtraHosts)])].filter(
    (host): host is string => host !== null,
  ),
);

if (DESCRIPTION_LINK_ALLOWED_HOSTS.length === 0) {
  // biome-ignore lint/suspicious/noConsole: build-time misconfiguration is otherwise invisible — main logger unreachable from renderer
  console.warn(
    '[renderer] build-description link allowlist is empty (API_URL missing or malformed at build time, DESCRIPTION_LINK_HOSTS unset) — every link in every build description renders as plain text',
  );
}
...
export const renderBuildDescriptionMarkdown = (description: string): string =>
