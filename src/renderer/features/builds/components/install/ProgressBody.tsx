...
export const ProgressBody = ({ active, paused, pausable, controls, key }: ProgressBodyProps) => {
  const { t } = useTranslation();
  const isDownloading = !paused && active.state === StepStates.ACTIVE;
  // Single byte-rate implementation for all three paths (install, repair, bundle):
  // a trailing window over the cumulative byte counter the events already carry.
  const speed = useByteSpeed(active.bytesDownloaded, isDownloading);
  const speedText = paused ? '' : formatSpeed(speed);
  ...
  const etaText =
    paused || indeterminate || !showBytes ? '' : formatEta(computeEtaSeconds(remaining, speed));
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <div className="relative h-9 w-full overflow-hidden rounded-md bg-surface-2">
        ...
        <div className="relative z-10 flex h-full items-center justify-between gap-3 px-3">
          ...
          >
            ...
            {paused ? t('builds.paused') : phaseLabel}
          </span>
          {!indeterminate && (
            <span
              className={cn(
      ...
      {stalled && <p className="text-eyebrow text-text-mute">{t('builds.progressStalled')}</p>}

      {controls && (
        <div className="flex gap-2">
          <ProgressControls kind={controls} paused={paused} pausable={pausable} key={key} />
        </div>
      )}
    </div>
  );
