  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-edge-md bg-surface-1/40 px-6 py-12 text-center">
      ...
      <div className="flex max-w-sm flex-col gap-1">
        <p className="text-body-med text-text-hi">{t('builds.myBuildsEmptyTitle')}</p>
        <p className="text-caption text-text-mute">{t('builds.myBuildsEmptyDescription')}</p>
      </div>
      ...
      return (
        <div className="flex flex-1 flex-col items-center justify-center gap-4 py-20 text-center">
          ...
          <p className="max-w-sm text-body text-text-mute">
            {t('builds.noResults', { query: search.trim() })}
          </p>
          <Button variant="secondary" onClick={() => setSearch('')}>
            <X className="size-4" />
            {t('builds.clearSearch')}
          </Button>
        </div>
      );
    }
      ...
      <div className="flex flex-col gap-8">
        ...
        {showOfficial && (
          <section className="flex flex-col gap-3.5">
            <SectionHeading
              className="mb-0"
              aside={<span className="text-caption text-text-mute">{t('builds.curatedBy')}</span>}
            >
              {t('builds.official')}
            </SectionHeading>
            <BuildGrid
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative w-full max-w-sm">
            ...
            <Input
              ...
              aria-label={t('builds.searchPlaceholder')}
              className="h-10 rounded-md border-edge bg-surface-1 pl-9"
            />
          </div>
          <Segmented<BuildViewMode>
            ariaLabel={t('builds.viewToggleAria')}
            value={viewMode}
            onChange={setViewMode}
            options={[
              { value: 'grid', icon: LayoutGrid, ariaLabel: t('builds.viewGrid') },
              { value: 'list', icon: List, ariaLabel: t('builds.viewList') },
            ]}
          />
        </div>
        {renderBody()}
