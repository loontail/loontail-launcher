...
const ErrorSurface = ({ message, children }: { message: string | null; children: ReactNode }) => (
...
export const PlayButton = ({ item }: PlayButtonProps) => {
  ...
  const rawPersistedLoader = settings?.clients[key]?.loader ?? null;
  ...
  const beginInstall = (loader?: LoaderChoice): Promise<void> =>
    install.mutateAsync({ key, ...(loader ? { loader } : {}) });
  ...
  if (action === PlayButtonActions.PROGRESS && progress) {
    return (
      <>
        <InstallProgress key={key} view={progress} />
        {loaderModal}
      </>
    );
  }
  ...
  if (action === PlayButtonActions.BUNDLE_ERROR && bundle.error) {
    return (
      <ErrorSurface message={localizeBundleError(bundle.error.code, bundle.error.message, t)}>
        <ActionButton
          onClick={() => void startBundle.mutateAsync({ key })}
  ...
  switch (action) {
    case PlayButtonActions.STATUS_PENDING:
      return <SpinnerButton label={t('builds.checking')} />;

    case PlayButtonActions.UNINSTALLING:
      return <SpinnerButton label={t('builds.uninstalling')} />;

    case PlayButtonActions.LAUNCHING:
      return (
        <ActionButton
          variant="muted"
          onClick={() => void cancel.mutateAsync(key)}
          disabled={cancel.isPending}
          title={t('builds.launching')}
        >
          <X size={16} />
          {t('builds.cancel')}
        </ActionButton>
    ...
    case PlayButtonActions.REPAIRING:
      return <SpinnerButton label={t('builds.repairing')} />;
    ...
    case PlayButtonActions.RUNNING:
      return (
        <div className="flex items-center gap-3">
          ...
          <ActionButton variant="muted" onClick={() => void stop.mutateAsync(key)}>
            <Square size={16} />
            {t('builds.stop')}
          </ActionButton>
        </div>
    ...
    case PlayButtonActions.PLAY:
      return (
        <ActionButton onClick={() => void launch.mutateAsync(key)} disabled={launch.isPending}>
          <Play size={16} />
          {t('builds.play')}
        </ActionButton>
      );
    ...
    case PlayButtonActions.UNVERIFIED:
      return (
        <ActionButton
          onClick={() => void launch.mutateAsync(key)}
          disabled={!folderReady || launch.isPending}
          title={folderReady ? undefined : t('builds.setInstallFolder')}
        >
          <RefreshCw size={16} />
          {t('builds.retry')}
        </ActionButton>
