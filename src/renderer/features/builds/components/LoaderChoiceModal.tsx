...
const LOADER_OPTIONS: ReadonlyArray<{
  ...
  { value: LoaderChoices.FORGE, labelKey: 'builds.loaderModal.forge' },
  { value: LoaderChoices.FABRIC, labelKey: 'builds.loaderModal.fabric' },
];
...
export const LoaderChoiceModal = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      ariaLabel={t('builds.loaderModal.title')}
      className="max-w-md"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 flex-col gap-1">
          <h2 className="truncate text-h2 text-text-hi">{t('builds.loaderModal.title')}</h2>
          <p className="text-caption text-text-mute">
            {t('builds.loaderModal.subtitle', { name: clientTitle })}
          </p>
        </div>
        <button
          type="button"
